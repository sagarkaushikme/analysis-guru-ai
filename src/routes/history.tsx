        ...
        {rows.length === 0 ? (
          ...
          <div className="mt-8 overflow-hidden rounded-2xl border border-border bg-card">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((r) => {
                  const c = r.trade_score >= 7 ? "text-success bg-success/10" : r.trade_score >= 5 ? "text-warning bg-warning/10" : "text-danger bg-danger/10";
                  return (
                    <tr key={r.id} onClick={() => { store.setCurrent(r); nav({ to: "/dashboard" }); }} className="cursor-pointer border-t border-border transition-colors hover:bg-background/40">
                      <td className="px-5 py-4 text-muted-foreground">{new Date(r.date).toLocaleDateString()}</td>
                      <td className="px-5 py-4 font-semibold">{r.instrument}</td>
                      <td className="px-5 py-4"><span className={`rounded-md px-2 py-1 text-xs font-semibold ${c}`}>{r.trade_score}/10</span></td>
                      <td className="px-5 py-4 text-muted-foreground">{r.pattern_name}</td>
                      <td className="px-5 py-4 text-right"><Button size="sm" variant="outline">View</Button></td>
                    </tr>
                  );
                })}
              </tbody>
